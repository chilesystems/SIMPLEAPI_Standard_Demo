...
namespace SIMPLEAPI_Demo
{
    public partial class ConsultaEstadoDTE : Form
    {
        ...
        private async void botonConsultar_Click(object sender, EventArgs e)
        {
            ...
            try 
            {
                var responseEstadoDTE = await handler.ConsultarEstadoDTEAsync(radioProduccion.Checked ? AmbienteEnum.Produccion: AmbienteEnum.Certificacion, $"{textRUTReceptor.Text}-{textDVReceptor.Text}", tipoDTE, folio, dateFechaEmision.Value.Date, total, checkIsBoletaCertificacion.Checked);
